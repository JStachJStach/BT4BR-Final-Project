#include "../headers/Tile_old.h"

#include "../headers/RandomUtils.h"
int Tile_old::_tileID = 0;
int Tile_old::_rabbitCount = 0;
int Tile_old::_foxCount = 0;
int Tile_old::_grassCount = 0;
 ...
 * Default Constructor
 */
Tile_old::Tile_old() : _color(BLACK)
{
}
 ...
 */
Tile_old::Tile_old(const std::string& nameIn, const Color& color) : _name(nameIn), _color(color)
{
    _tileID++;
    if (_name == "Grass")
    {
 ...
 */
void Tile_old::draw(const std::array<int, 2>& currentPosition) const
{
 ...
 * @return New position
 */
std::array<int, 2> Tile_old::act(const std::array<int, 2> &currentPosition, std::map<std::array<int, 2>, Tile_old> &tileMap)
{   
    // Satiation system
    if (this->_name == "Rabbit")
    {
        ...
        if (this->_satiation > rabbitReproductionSat)
        {
            for (auto pos : RandomUtils::positionsAdjacent(currentPosition))
            {
                ...
                {
                    const Tile_old* tile = new Tile_old("Rabbit", GRAY);
    ...
    if (this->_name == "Fox")
    {
        ...
        if (this->_satiation > foxReproductionSat)
        {
            for (auto pos : RandomUtils::positionsAdjacent(currentPosition))
            {
                if (!tileMap.contains(pos))
                {
                    const Tile_old* tile = new Tile_old("Fox", ORANGE);
                    tileMap[std::array{pos[0], pos[1]}] = *tile;
                    delete tile;
                    break;
                }
...
std::array<int, 2>  Tile_old::move_if_target_in_sight(const std::array<int,2> pos, const std::array<int, 2> &currentPosition)
{
 ...
 */
void Tile_old::set_color(const Color & newColor)
{
    _color = newColor;
}
/**
 ...
 */
void Tile_old::set_state(const TileState & newState)
{
    _state = newState;
}
/**
 ...
 */
void Tile_old::set_name(const std::string & newName)
{
    _name = newName;
}
 ...
 */
TileState Tile_old::get_state() const
{
    return _state;
}
/**
 ...
 */
Color Tile_old::get_color() const
{
    return _color;
}
/**
 ...
 * @return current name
 */
std::string Tile_old::get_name() const
{
    return _name;
}
